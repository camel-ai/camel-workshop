...
try:
    ...
    import camel
    print("camel-ai is already installed.")
except ImportError:
    # If the import fails, install the library
    print("camel-ai not found. Installing...")
    try:
        subprocess.check_call([sys.executable, "-m", "pip", "install", "camel-ai[all]==0.2.73"])
        print("camel-ai installed successfully.")
        # Now that it's installed, you can import it
        import camel
    except subprocess.CalledProcessError as e:
        print(f"Failed to install camel-ai. Error: {e}")
...
MUST use this as the current date.
    ...
    human_task = Task(
        content=(
            """
Analyze the Saudi smart city industry to support the planning of my company. Provide a comprehensive market overview, including current trends, growth projections, and relevant regulations. Identify the top 5 major opportunities, gaps, or underserved segments within the market. Present all findings in a well-structured, professional HTML report.
            """
        ),
        id='0',
    )
